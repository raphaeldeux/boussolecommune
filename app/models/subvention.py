...
def get_years(ville_id=1, thematique=None):
    with get_db() as conn:
        if thematique:
            rows = conn.execute(
                "SELECT DISTINCT annee FROM subventions WHERE ville_id = ? AND thematique = ? ORDER BY annee DESC",
                (ville_id, thematique),
            ).fetchall()
        else:
            rows = conn.execute(
...
def get_previous_year(annee, ville_id=1, thematique=None):
    ...
    with get_db() as conn:
        if thematique:
            row = conn.execute(
                "SELECT MAX(annee) AS prev FROM subventions WHERE ville_id = ? AND thematique = ? AND annee < ?",
                (ville_id, thematique, annee),
            ).fetchone()
        else:
            row = conn.execute(
                "SELECT MAX(annee) AS prev FROM subventions WHERE ville_id = ? AND annee < ?",
                (ville_id, annee),
            ).fetchone()
    ...
    with get_db() as conn:
        if thematique:
            rows = conn.execute(
                ...
                "FROM subventions WHERE annee = ? AND ville_id = ? AND thematique = ? ORDER BY montant DESC",
    ...
    with get_db() as conn:
        if thematique:
            rows = conn.execute(
                ...
                "WHERE annee = ? AND ville_id = ? AND thematique = ? GROUP BY domaine ORDER BY total DESC",
                (annee, ville_id, thematique),
            ).fetchall()
        else:
            rows = conn.execute(
...
def get_total(annee, ville_id=1, thematique=None):
    with get_db() as conn:
        if thematique:
            row = conn.execute(
                "SELECT SUM(montant) AS total FROM subventions WHERE annee = ? AND ville_id = ? AND thematique = ?",
                (annee, ville_id, thematique),
            ).fetchone()
        else:
            row = conn.execute(
...
def get_by_id(id_):
    with get_db() as conn:
        row = conn.execute("SELECT * FROM subventions WHERE id = ?", (id_,)).fetchone()
...
def update(id_, annee, nom_beneficiaire, domaine, montant, commentaire="", thematique="lien_social"):
    with get_db() as conn:
        conn.execute(
            "UPDATE subventions SET annee=?, nom_beneficiaire=?, domaine=?, thematique=?, montant=?, commentaire=? "
            "WHERE id=?",
